...
def load_predictions(database):
  ...
  joined_data = sqlctxt.sql("select * from {}.pxo_navnow_mtmi_funds_predicted_1".format(database_m)).coalesce(1)
  ...
  fund_results = sqlctxt.sql("select * from {}.pxo_navnow_mtmi_funds_predicted".format(database_m)).coalesce(1)  
  ...
  predictions = sqlctxt.sql("select client_identifier,bin,weekday,red_1,predicted,signal \
                            from {}.pxo_navnow_mtmi_predicted".format(database_m)).coalesce(1).dropna()
  
def initial_run():  
    global first_run,joined_data
    if (first_run != 1):
      current_date = str(datetime.datetime.date(datetime.datetime.now()))
      origin_data = sqlctxt.sql("select * from {}.pxo_navnow_mtmi_clientlevel \
      ...
      if (origin_data.rdd.isEmpty() is True):  
        ...
        origin_data.coalesce(1).select('client_identifier','nav_date','bin','count','cumsum','predicted','red_1','error','signal')\
                               .write.mode("overwrite").insertInto("{}.pxo_navnow_mtmi_clientlevel".format(database_m))                
        ...
        updated_data.coalesce(1).select('fund_id','nav_date','client_name','predicted_time','max_predictedtime',\
                                ...
                                .write.insertInto("{}.pxo_navnow_mtmi_fundlevel".format(database_m))
...
def empty_rdd(rdd): 
  ...
  if (rdd.isEmpty() is True and time.strftime("%H:%M:%S") >= '16:00:00'):    
    ...
    origin_data = sqlctxt.sql("select * from {}.pxo_navnow_mtmi_clientlevel where nav_date = '{}'".format(database_m,current_date))
    ...
    old_data = f.broadcast(old_data).withColumn('min_bin', f.min('new_prediction').over(w))\
                             .where(f.col('new_prediction') == f.col('min_bin')).drop('min_bin')    
    processed_data = sqlctxt.sql("select * from {}.pxo_navnow_mtmi_fundlevel where nav_date ='{}' \
...
def process(rdd):  
    ...
    if (rdd.isEmpty() is False and time.strftime("%H:%M:%S") >= '16:00:00'):    
      ...
      data.coalesce(1).select('nav_date','fund_id','event_ts')\
                      .write.insertInto("{}.pxo_navnow_mtmi_raw_data".format(database))
      ...
      processed_data = sqlctxt.sql("select * from {}.pxo_navnow_mtmi_fundlevel where actual_completion_time != 'NA'".format(database_m)).coalesce(1).drop_duplicates()      
